...
for day in days:
    ...
    for j in range(1, len(lessons)):
        ...
        if lesson.find('span','schedule-lesson') != None:
            if 'L1' in lesson.find('span','schedule-lesson').text.strip() or 'L2' in lesson.find('span','schedule-lesson').text.strip():
                continue
            else:
