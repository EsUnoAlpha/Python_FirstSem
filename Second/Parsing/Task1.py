...
tracks = soup.find_all("div", "d-track__name")
artists = soup.find_all("div", "d-track__meta")

chart_data = {}

for i in range(len(tracks)):
    track_name = tracks[i].text.strip()
    artist_name = artists[i].text.strip()
    chart_data[i+1] = ({'Исполнитель' : artist_name}, {'Трек' : track_name})
